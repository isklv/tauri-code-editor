 ...
 *
 * Usage:
 *   node scripts/release.js               # Run release pipeline on current version
 *   node scripts/release.js patch         # Bump patch (0.1.0 -> 0.1.1) and release
 *   node scripts/release.js minor         # Bump minor (0.1.0 -> 0.2.0) and release
 *   node scripts/release.js 0.2.0         # Set specific version and release
 *   node scripts/release.js --no-apk      # Run checks & tests without building APK
 */
...
if (versionArg) {
  ...
  execSync(`node scripts/version.js ${versionArg}`, { cwd: rootDir, stdio: 'inherit' });
}
...
execSync('node scripts/security-check.js', { cwd: rootDir, stdio: 'inherit' });
...
step(currentStep++, totalSteps, 'Building Web Frontend Assets...');
...
if (!skipApk) {
  ...
  execSync('node scripts/build-apk.js', { cwd: rootDir, stdio: 'inherit' });
}
